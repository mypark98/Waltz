...
<body>
    ...
    <form name="readForm" action = "/contest/contestBoardUpdate" role="form" method="get">
         ...
         <div id="contents" >
                    ...
                    <div id="detail3_func" style="text-decoration: none;">
                       ...
                       <c:choose>
                        <c:when
                           ...
                           <td>
                              <input type="button" value="수정" id="updateSubmit" style="position:relative; bottom:-20px; left:80px;">
                              <input type="button" onclick="checkPostDelete()" value="삭제" style="position:relative; bottom:-20px; left:80px;">
                        
                           </td>
                        </c:when>
                        <c:otherwise>
                        <button type="button" id="list_btn" style="position:relative; right:-100px; bottom:-20px;">목록으로</button>
                         <p />
                        </c:otherwise>
                       </c:choose>
                     </div>
            ...
            <div style="margin-top:30px;"> 
	            ...
	            <c:if test="${authInfo.id ne null}">
	            	<form action="/contest/commentBoardInsert" method="post" name="commentForm">
				                    ...
				                    <div id="cohead${commentList.CO_NUM}" >
					                    ...
					                    <div>
					                    	 <c:if test="${(authInfo.id eq commentList.MEM_ID) || (authInfo.manager eq 1)}">
												...
												<div style="display:inline-flex; margin-top:-30px; margin-left:20px;  width:200px; height:100px;">
														...
														<p style="font-size:13px; color: rgb(184, 177, 177); margin-top:5px;"><a class="disableLink" style="color: rgb(184, 177, 177);" href="<c:url value='/contest/commentBoardDelete?PO_NUM=${detail.PO_NUM}&CO_NUM=${commentList.CO_NUM}&page=${scri.page}&perPageNum=${scri.perPageNum}&searchType=${scri.searchType}&keyword=${scri.keyword}'/>" role="button" onclick="checkDelete()">삭제</a></p>
   ...
   // 댓글 삭제
      function checkDelete(conum) {
         ...
         if (chk == true) {
            self.location = "/contest/commentBoardDelete?"
                  + "PO_NUM=${detail.PO_NUM}"
                  + "&CO_NUM=" + conum
                  + "&page=${scri.page}" 
      ...
      $("#list_btn").click(function(){
         self.location = "/contest/contestBoardList?"
                     + "page=${scri.page}&perPageNum=${scri.perPageNum}"
                     + "&searchType=${scri.searchType}&keyword=${scri.keyword}";
      });
   ...
   // 간편 유효성 검사
      ...
      function updateCommentBtn(CO_NUM, MEM_ID, CO_CONTENT, PO_NUM, page, perPageNum, searchType, keyword){ 
         $('#cohead'+ CO_NUM).html(            
            "<form action='/contest/commentUpdate?CO_NUM="+CO_NUM+"&PO_NUM="+PO_NUM+"' name='commentEditForm' method='get' style='margin-left:20px; margin-top:-20px;'><br>"
            ...
            + "<textarea id='commentContent' name='CO_CONTENT' cols='70' required>"+CO_CONTENT+"</textarea>" 
            + "<style>#commentContent{width:740px; height:80px; resize:none;} </style> <br>" 
